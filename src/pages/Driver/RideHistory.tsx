...
import { useDriverRidesQuery } from '@/redux/features/rider/ride.api';
...
const RideHistory = () => {
    const { data: rides, isLoading } = useDriverRidesQuery(undefined);

    const driverRides = rides?.data || [];
    ...
    //     setCurrentPage(1);
    // }, [searchQuery, statusFilter, fareRange, driverRides, sortField, sortDirection]);

    // Pagination
    // const totalPages = Math.ceil(filteredRides.length / itemsPerPage);
    const totalPages = Math.ceil(driverRides.length / itemsPerPage);
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div className="flex items-end">
                                <div className="text-sm text-muted-foreground">
                                    {/* TODO */}
                                    {/* Showing {filteredRides.length} of {driverRides.length} rides */}
                                    Showing 10 of {driverRides.length} rides
                                </div>
                            </div>
                        </div>
                    </div>
                                    ...
                                    {driverRides.length > 0 ? (
                                        ...
                                        <tr>
                                            <td colSpan={6} className="p-8 text-center text-muted-foreground">
                                                {driverRides.length === 0 ? 'No rides found.' : 'No rides found matching your filters.'}
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
                    ...
                    {totalPages > 1 && (
                        <div className="flex items-center justify-between">
                            <div className="text-sm text-muted-foreground">
                                ...
                                Page {currentPage} of {totalPages} • {driverRides.length} total rides
                            </div>
